...
export function createAxisFilterMapper({
  ...
  return (axis: AxisConfig, axisIndex: number): ExtremumFilter | null => {
    ...
    return createContinuousScaleGetAxisFilter(
      scaleType,
      getAxisExtrema(axis, direction, seriesConfig, axisIndex, formattedSeries),
      zoom.start,
      zoom.end,
      direction,
      axis.data,
...
export function createDiscreteScaleGetAxisFilter(
  ...
  return function filterAxis(value, dataIndex) {
    ...
    // Use floor/ceil to ensure that we only remove data points that are completely outside the zoom range.
    return dataIndex >= Math.floor(minVal) && dataIndex < Math.ceil(maxVal);
  };
}
